...
from deployment_readiness import (
    load_corpus, held_out_range, build_clean_windows, build_window_arrays,
    run_window_model, apply_injection, node_fully_real,
    CONFIRM_MAGNITUDE, N_SECTION3_WINDOWS, INJECTION_SHAPES,
    SECTION4_NODES, SECTION4_MAGNITUDES,
)

# prior numbers from the test-path deployment battery, for the before/after tables
PRIOR_FP_RATE = {"north_fork_0": 0.267, "south_fork_2": 0.167, "south_fork_1": 0.312, "oxford": 0.143}
PRIOR_STEP_DETECTION_150 = {"north_fork_0": 0.20, "south_fork_2": 0.17, "south_fork_1": 0.00, "oxford": 0.17}
...
def _score_node(predictions, targets, node_idx, cond_idx, site, real_ts, rain_series, metadata):
    ...
    rain_mult, _ = ad._rain_multipliers(
        real_ts, rain_series, Config.RAIN_WINDOW_HOURS, Config.RAIN_THRESHOLD_MULTIPLIER,
...
def section2_livepath(model, metadata, edge_index):
    ...
    results = {site: {"n_eval": 0, "n_flagged": 0, "n_flagged_rule1": 0, "n_flagged_rule2": 0} for site in node_names}
    ...
    for window_ts in windows:
        ...
        predictions, targets = run_window_model(model, edge_index_local, normalized, node_mask, num_nodes)
        ...
        for site in node_names:
            ...
            flagged, rule1, rule2 = _score_node(preds_real, tgts_real, node_idx, cond_idx, site, real_ts, rain_series, metadata)
    ...
    print(f"{'node':15s} {'evaluated':>10s} {'flagged':>8s} {'FP rate':>9s} {'rule1':>7s} {'rule2':>7s}")
    for site in node_names:
        r = results[site]
        if r["n_eval"] == 0:
...
def section3_livepath(model, metadata, edge_index, magnitude=CONFIRM_MAGNITUDE, n_windows=N_SECTION3_WINDOWS):
    print("=" * 70)
        ...
        for target_site in node_names:
            ...
            for shape in INJECTION_SHAPES:
                ...
                flagged, rule1, rule2 = _score_node(
                    predictions, targets, node_idx, cond_idx, target_site, target_ts, rain_series, metadata
                )
                ...
                loc = localization.setdefault(target_site, {
                    "any_injections": 0, "any_neighbor_checks": 0, "any_neighbor_false_flags": 0,
                    "rule2_injections": 0, "rule2_neighbor_checks": 0, "rule2_neighbor_false_flags": 0,
                })
                ...
                for nb_site in neighbor_sites:
                    ...
                    nb_flagged, nb_rule1, nb_rule2 = _score_node(
                        predictions, targets, nb_idx, cond_idx, nb_site, target_ts, rain_series, metadata
                    )
                    loc["any_neighbor_checks"] += 1
                    if nb_flagged:
                        loc["any_neighbor_false_flags"] += 1
                    if rule2["flagged"]:  # only counts toward the rule2-specific stat when rule2 fired on the target
    ...
    for site in node_names:
        ...
        for shape in INJECTION_SHAPES:
            ...
            if d is None or d["tested"] == 0:
                ...
                cells.append(f"{d['flagged']}/{d['tested']} ({rate:.0%}, r1={d['rule1']} r2={d['rule2']})")
    ...
    print("-" * 80)
    print(f"{'injected node':15s} {'any: checks':>12s} {'any: false':>11s} {'any rate':>9s}   "
    ...
    for site in node_names:
        ...
        r2_rate = (
            ...
            if loc["rule2_neighbor_checks"] > 0 else float("nan")
        )
        r2_checks_str = str(loc["rule2_neighbor_checks"]) if loc["rule2_neighbor_checks"] > 0 else "0 (rule2 never fired)"
        print(
...
def section4_livepath(model, metadata, edge_index, nodes=SECTION4_NODES, magnitudes=SECTION4_MAGNITUDES, n_windows=N_SECTION3_WINDOWS):
    print("=" * 70)
    ...
    for window_ts in test_windows:
        ...
        for site in nodes:
            ...
            for magnitude in magnitudes:
                ...
                d = results[site].setdefault(magnitude, {"tested": 0, "flagged": 0, "rule1": 0, "rule2": 0})
                d["tested"] += 1
                if flagged:
                    d["flagged"] += 1
                if rule1["flagged"]:
...
def before_after(section2_results, section3_detection):
    ...
    for site in ("north_fork_0", "south_fork_2", "south_fork_1", "oxford"):
        ...
        delta_str = f"{(after - before):+.1%}" if before is not None and after is not None else "n/a"
